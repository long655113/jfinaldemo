...
public enum GetContentWay {
    ...
    ID(1, "id"), CLASS(2, "class") {
                ...
                public String getContent(String url, String key) {
                    String content = null;
                    try {
                        Document root = JsoupUtil.getDoc(url);

                        //如果访问失败，再试3次
                        if (root == null) {
                            for (int i = 0; i < 3; i++) {
                                root = JsoupUtil.getDoc(url);
                            }
                        }
    ...
    private GetContentWay(int value, String name) {
        this.value = value;
        this.name = name;
    }

    public static GetContentWay getIndexWayBayValue(int value) {

        for (GetContentWay way : GetContentWay.values()) {
            if (way.value == value) {
                return way;
            }
        }
